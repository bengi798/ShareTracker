...
public class CreateCryptoTradeCommandValidator : AbstractValidator<CreateCryptoTradeCommand>
{
    public CreateCryptoTradeCommandValidator()
    {
        ...
        RuleFor(x => x.CoinSymbol)
            .NotEmpty()
            .MaximumLength(20)
            .Matches(@"^[A-Za-z0-9]+$").WithMessage("Coin symbol must be alphanumeric.");

        RuleFor(x => x.Network)
            .MaximumLength(50)
            .When(x => x.Network is not null);
