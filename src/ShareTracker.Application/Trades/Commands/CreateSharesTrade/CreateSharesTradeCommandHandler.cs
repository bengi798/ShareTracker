...
public class CreateSharesTradeCommandHandler : IRequestHandler<CreateSharesTradeCommand, TradeDto>
{
    private readonly ITradeRepository _trades;
    private readonly IUnitOfWork _uow;
    private readonly ICurrentUserService _currentUser;

    public CreateSharesTradeCommandHandler(
    ...
    public async Task<TradeDto> Handle(CreateSharesTradeCommand request, CancellationToken cancellationToken)
    {
        ...
        if (tradeType == TradeType.Sell)
        {
            ...
            var matchingTrades = existing
                .OfType<SharesTrade>()
                .Where(t => string.Equals(t.Ticker.Value, request.Ticker, StringComparison.OrdinalIgnoreCase)
                         && t.Exchange == exchange)
                .ToList();
            ...
            if (request.NumberOfUnits > netPosition)
                ...
                {
                    ["NumberOfUnits"] = [$"Insufficient position. You have {netPosition:G} available unit(s) of {request.Ticker.ToUpperInvariant()} ({exchange})."]
                });
