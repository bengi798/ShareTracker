...
public class CreateBondTradeCommandHandler : IRequestHandler<CreateBondTradeCommand, TradeDto>
{
    private readonly ITradeRepository _trades;
    private readonly IUnitOfWork _uow;
    private readonly ICurrentUserService _currentUser;

    public CreateBondTradeCommandHandler(
    ...
    public async Task<TradeDto> Handle(CreateBondTradeCommand request, CancellationToken cancellationToken)
    {
        ...
        if (tradeType == TradeType.Sell)
        {
            ...
            var matchingTrades = existing
                .OfType<BondTrade>()
                .Where(t => string.Equals(t.BondCode, request.BondCode, StringComparison.OrdinalIgnoreCase))
                .ToList();
            ...
            if (request.NumberOfUnits > netPosition)
                ...
                {
                    ["NumberOfUnits"] = [$"Insufficient position. You have {netPosition:G} available unit(s) of {request.BondCode.ToUpperInvariant()}."]
                });
